// this is the code with couier chat
  ...
  const fetchApplicants = async () => {
    try {
      ...
      if (res.status === 1) {
        ...
        // Get complete job details ALWAYS
        const detailsRes = await getJobDetails(id, "customer", token);

        if (detailsRes.status === 1) {
          setJob(detailsRes.payload.job);
        } else {
          // fallback to job from applicants API
          setJob(res.payload.job);
          toast.error(detailsRes.msg || "Failed to load complete job details.");
        }
      } else {
            ...
            {applicants.length === 0 ? (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  You will see courier applications here when someone applies.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
