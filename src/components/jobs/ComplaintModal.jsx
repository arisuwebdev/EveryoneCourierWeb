...
export default function ComplaintModal({
  ...
  // Load existing complaint when editing
  useEffect(() => {
    if (open && isEditing && complaint) {
      setSelectedComplaint(complaint.complaint_type || "");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b px-4 py-3">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              {isEditing ? "Edit Complaint" : "Dispute Resolution"}
            </h2>
          ...
          <div className="space-y-1.5">
            {complaints.map((item) => (
              <button
                key={item.value}
                type="button"
                onClick={() => setSelectedComplaint(item.value)}
                className={`w-full rounded-md border px-3 py-2 text-left transition ${
                  selectedComplaint === item.value
                    ? "border-red-500 bg-red-50"
                    : "border-slate-200 hover:border-red-300 hover:bg-slate-50"
                }`}
              >
